...
auth.post("/register", async (req, res) => {
  ...
  const userDatas = await User.findAll({
    //해당 이메일 존재여부를 확인하기 위해 우선적으로 요청 email과 동일한 것을 읽어온다.
    where: {
      email: email,
    },
  });
  if (!email && !password) {
    //요청에 email, password 없는 경우
    return res.json("정상적인 요청이 아닙니다.");
  }
  if (userDatas.some((user) => user.email === email)) {
    //이미 존재하는 이메일일 경우 작성 불가.
    return res.json({
  ...
  const userCreate = await User.create({
    //기존회원이 아닐 경우, 요청 email과 pw을 지닌 row insert
    email: email,
...
auth.post("/login", async (req, res) => {
  ...
  const userDatas = await User.findAll({
    //요청한 것과 동일한 email과 pw를 지닌 행을 읽어온다.
    where: {
      email: email,
    },
  });
  ...
  if (userDatas.length === 0) {
    //!userDatas를 사용해보려고 했으나, 해당 유저가 없을 때 즉, 빈 객체일 때도 자꾸 true로 인식되어 일단 리뷰 전 방식으로 택하였습니다 ㅜㅜ
    return res.json({
